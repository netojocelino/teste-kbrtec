...
<main class="max-w-7xl mx-2 lg:mx-auto">
    ...
    <div class="flex justify-center gap-2">
        <p class="text-center">
            @if (optional(data_get($championshipGroup,0))->belt == 'black')
                <span class="bg-black px-3 rounded-md text-white">
                    Faixa Preta
                </span>
                @elseif (optional(data_get($championshipGroup,0))->belt == 'brown')
        ...
        <p class="flex items-center justify-center text-gray-500 mb-3">
            ...
            @if (optional(data_get($championshipGroup,0))->weight == 'light')
                Peso Leve
            @elseif (optional(data_get($championshipGroup,0))->belt == 'strong')
                Peso Pesado
        @endif
        </p>
        <p class="flex items-center justify-center text-gray-500 mb-3">
    ...
    <div class="mt-4 flex gap-4">
        ...
        @foreach ($championshipGroup as $item)
    @if ($firstElement && $item->match_level == $index)
        ...
        @php
                $firstElement = false;
        @endphp
    @elseif (!$firstElement && $item->match_level != $index)
    ...
    @endif
                    ...
                    <p class="absolute -top-6 text-gray-700 font-bold">Disputa {{ $item->match_number }}</p>
                    <ul class="border-t-4 border-b-4 border-blue-700">
                        <li class="odd:bg-gray-200 even:bg-gray-100 flex items-stretch border-b border-gray-100">
                            <p class="p-2 block w-full text-sm font-bold {{ $item->winner_athlete_id == $item->first_athlete_id ? 'text-gray-900' : 'text-gray-400'}}">
                                {{ optional($item->firstAthlete)->full_name }}
                                <span class="block text-blue-700 font-normal text-xs">
                                    {{ optional($item->firstAthlete)->team }}
                                </span>
                            </p>
                        </li>
                        <li class="odd:bg-gray-200 even:bg-gray-100 flex items-stretch border-b border-gray-100">
                            <p class="p-2 block w-full text-sm font-bold {{ $item->winner_athlete_id == $item->second_athlete_id ? 'text-gray-900' : 'text-gray-400'}}">
                                {{ optional($item->secondAthlete)->full_name }}
                                <span class="block text-blue-700 font-normal text-xs">
                                    {{ optional($item->secondAthlete)->team }}
                                </span>
                            </p>
                        </li>
                    </ul>
